...
if(option.safe&&funct['(global)']&&typeof predefined[t]!=='boolean'){warning('ADsafe global: '+t+'.',token);}
...
for(i=1;i<functions.length;i+=1){f=functions[i];cl=[];ex=[];va=[];un=[];ou=[];gl=[];la=[];for(k in f){if(f.hasOwnProperty(k)&&k.charAt(0)!=='('){v=f[k];switch(v){case'closure':cl.push(k);break;case'var':va.push(k);break;case'exception':ex.push(k);break;case'outer':ou.push(k);break;case'global':gl.push(k);break;case'label':la.push(k);break;case'unused':un.push(k);break;}}}
...
o.push(m+'<br>*/</pre>');}
o.push('</div>');}
return o.join('');};itself.edition='2009-06-14';return itself;}());
