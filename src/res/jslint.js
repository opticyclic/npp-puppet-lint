...
function comment(snippet){if(comments_off||src||(xmode&&xmode!=='script'&&xmode!=='style'&&xmode!=='styleproperty')){warn_at('unexpected_comment',line,character);}else if(xmode==='script'&&/<\//i.test(source_row)){warn_at('unexpected_a',line,character,'<\/');}else if(option.safe&&ax.test(snippet)){warn_at('dangerous_comment',line,character);}else if(!option.todo&&tox.test(snippet)){warn_at('todo_comment',line,character);}}
...
while(xmode==='outer'){i=source_row.search(ox);if(i===0){break;}else if(i>0){character+=i;source_row=source_row.slice(i);break;}else{if(!next_line()){return it('(end)','');}}}
...
if(option.safe){option.browser=option['continue']=option.css=option.debug=option.devel=option.evil=option.forin=option.newcap=option.nomen=option.on=option.rhino=option.sloppy=option.sub=option.undef=option.windows=false;delete predefined.Array;delete predefined.Date;delete predefined.Function;delete predefined.Object;delete predefined['eval'];add_to_predefined({ADSAFE:false,lib:false});}}
...
switch(that.id){case'++':case'--':if(!option.plusplus){warn('unexpected_a',that);}else if((!that.first.identifier||that.first.reserved)&&that.first.id!=='.'&&that.first.id!=='['){warn('bad_operand',that);}
break;default:if(that.first.arity==='prefix'||that.first.arity==='function'){warn('unexpected_a',that);}}
...
break;case'false':case'function':case'Infinity':case'NaN':case'null':case'true':case'undefined':case'void':case'(number)':case'(regexp)':case'(string)':case'{':warn(message||bundle.weird_condition,node);break;case'(':if(node.first.id==='new'||(node.first.string==='Boolean')||(node.first.id==='.'&&numbery[node.first.second.string]===true)){warn(message||bundle.weird_condition,node);}
break;}
return node;}
...
var right=expression(100);if(are_similar(left,right)||((left.id==='(string)'||left.id==='(number)')&&(right.id==='(string)'||right.id==='(number)'))){warn('weird_relation',that);}else if(left.id==='typeof'){if(right.id!=='(string)'){warn("expected_string_a",right,right.id==='(number)'?right.number:right.string);}else if(right.string==='undefined'||right.string==='null'){warn("unexpected_typeof_a",left,right.string);}}
...
return that;});prefix('void',function(that){that.first=expression(0);if(option.es5||strict_mode){warn('expected_a_b',that,'undefined','void');}else if(that.first.number!==0){warn('expected_a_b',that.first,'0',artifact(that.first));}
return that;});bitwise('|',70);bitwise('^',80);bitwise('&',90);relation('==','===');relation('===');relation('!=','!==');relation('!==');relation('<');relation('>');relation('<=');relation('>=');bitwise('<<',120);bitwise('>>',120);bitwise('>>>',120);infix('in',120,function(left,that){warn('infix_in',that);that.left=left;that.right=expression(130);return that;});infix('instanceof',120);infix('+',130,function(left,that){if(left.id==='(number)'){if(left.number===0){warn('unexpected_a',left,'0');}}else if(left.id==='(string)'){if(left.string===''){warn('expected_a_b',left,'String','\'\'');}}
...
if(left.id===right.id&&left.id==='(number)'){left.number%=right.number;left.thru=right.thru;return left;}
that.first=left;that.second=right;return that;});suffix('++');prefix('++');suffix('--');prefix('--');prefix('delete',function(that){one_space();var p=expression(0);if(!p||(p.id!=='.'&&p.id!=='[')){warn('deleted');}
that.first=p;return that;});prefix('~',function(that){no_space_only();if(!option.bitwise){warn('unexpected_a',that);}
that.first=expression(150);return that;});function banger(that){no_space_only();that.first=expected_condition(expression(150));if(bang[that.first.id]===that||that.first.assign){warn('confusing_a',that);}
return that;}
prefix('!',banger);prefix('!!',banger);prefix('typeof');prefix('new',function(that){one_space();var c=expression(160),n,p,v;that.first=c;if(c.id!=='function'){if(c.identifier){switch(c.string){case'Object':warn('use_object',token);break;case'Array':if(next_token.id==='('){p=next_token;p.first=this;advance('(');if(next_token.id!==')'){n=expression(0);p.second=[n];if(n.id!=='(number)'||next_token.id===','){warn('use_array',p);}
...
break;case'Date':case'RegExp':case'this':break;default:if(c.id!=='function'){v=c.string.charAt(0);if(!option.newcap&&(v<'A'||v>'Z')){warn('constructor_name_a',token);}}}}else{if(c.id!=='.'&&c.id!=='['&&c.id!=='('){warn('bad_constructor',token);}}}else{warn('weird_new',that);}
...
step_in();if(next_token.id!==')'){no_space();for(;;){edge();e=expression(10);if(left.string==='Boolean'&&(e.id==='!'||e.id==='~')){warn('weird_condition',e);}
p.push(e);if(next_token.id!==','){break;}
comma();}}
no_space();step_out(')',that);if(typeof left==='object'){if(left.string==='parseInt'&&p.length===1){warn('radix',left);}else if(left.string==='String'&&p.length>=1&&p[0].id==='(string)'){warn('unexpected_a',left);}
...
if(left.id==='.'){if(p.length>0&&left.first&&left.first.first&&are_similar(p[0],left.first.first)){if(left.second.string==='call'||(left.second.string==='apply'&&(p.length===1||(p[1].arity==='prefix'&&p[1].id==='[')))){warn('unexpected_a',left.second);}}
if(left.second.string==='toString'){if(left.first.id==='(string)'||left.first.id==='(number)'){warn('unexpected_a',left.second);}}}}
...
var value=expression(0);value.paren=true;no_space();step_out(')',that);if(value.id==='function'){switch(next_token.id){case'(':warn('move_invocation');break;case'.':case'[':warn('unexpected_a');break;default:warn('bad_wrap',that);}}
...
tally_property(e.string);break;default:if(option.safe){if((e.id!=='+'||e.arity!=='prefix')&&e.id!=='-'&&e.id!=='*'){warn('adsafe_subscript_a',e);}}}
step_out(']',that);no_space(prev_token,token);that.first=left;that.second=e;return that;},true);prefix('[',function(that){that.first=[];step_in('array');while(next_token.id!=='(end)'){while(next_token.id===','){warn('unexpected_a',next_token);advance(',');}
if(next_token.id===']'){break;}
indent.wrap=false;edge();that.first.push(expression(10));if(next_token.id===','){comma();if(next_token.id===']'&&!option.es5){warn('unexpected_a',token);break;}}else{break;}}
step_out(']',that);return that;},170);function property_name(){var id=optional_identifier(true);if(!id){if(next_token.id==='(string)'){id=next_token.string;if(option.safe){if(banned[id]){warn('adsafe_a');}else if(id.charAt(0)==='_'||id.charAt(id.length-1)==='_'){warn('dangling_a');}}
...
that.first.push(name);if(seen[i]===true){warn('duplicate_a',next_token,i);}
...
return this;});prefix('function',function(that){if(!option.anon){one_space();}
var id=optional_identifier();if(id){no_space();}else{id='';}
do_function(that,id);if(funct['(loopage)']){warn('function_loop');}
...
this.first=expression(0);if(this.first.assign){warn('unexpected_a',this.first);}}
...
function css_shadow(){for(;;){if(next_token.string==='inset'){advance();}
for(;;){if(!css_length()){break;}}
css_color();if(next_token.id!==','){break;}
advance(',');}
return true;}
...
function style_selector(){if(next_token.identifier){if(!Object.prototype.hasOwnProperty.call(html_tag,next_token.string)){warn('expected_tagname_a');}
...
function do_tag(tag,name,attribute){var i,script,x;src=false;if(stack.length>0){if(name==='html'){stop('unexpected_a',token,name);}
x=tag.parent;if(x){if(x.indexOf(' '+stack[stack.length-1].name+' ')<0){stop('tag_a_in_b',token,name,x);}}else if(x!==false&&!option.adsafe&&!option.fragment){i=stack.length;do{if(i<=0){stop('tag_a_in_b',token,name,'body');}
...
function html(){var attribute,attributes,is_empty,name,old_white=option.white,quote,tag_name,tag,value,wmode;xmode='html';xquote='';stack=null;for(;;){switch(next_token.string){case'<':xmode='html';advance('<');attributes={};tag_name=next_token;name=tag_name.string;advance_identifier(name);tag_name.name=name;if(!stack){stack=[];do_begin(name);}
tag=html_tag[name];if(typeof tag!=='object'){tag={parent:false};warn('unrecognized_tag_a',tag_name,name===name.toLowerCase()?name:name+' (capitalization error)');}else{is_empty=tag.empty;}
tag_name.type=name;for(;;){if(next_token.id==='/'){advance('/');if(next_token.id!=='>'){warn('expected_a_b',next_token,'>',artifact());}
break;}
...
warn('bad_name_a');}
option.white=false;spaces();attribute=next_token.string;option.white=old_white;advance();if(attribute!==attribute.toLowerCase()){warn('attribute_case_a',token);}
...
xmode='html';xquote='';advance(quote);value=false;}else if(attribute==='style'){xmode='scriptstring';advance('=');quote=next_token.id;if(quote!=='"'&&quote!=='\''){stop('expected_a_b',next_token,'"',artifact());}
...
advance();}else{value=true;}}
attributes[attribute]=value;do_attribute(attribute,value);}
do_tag(tag,name,attributes);if(!is_empty){stack.push(tag_name);}
xmode='outer';advance('>');break;case'</':xmode='html';advance('</');if(!next_token.identifier){warn('bad_name_a');}
name=next_token.string;advance();if(!stack){stop('unexpected_a',next_token,closetag(name));}
...
adsafe_top=true;tree=statements();begin.first=tree;itself.tree=begin;if(option.adsafe&&(tree.length!==1||aint(tree[0],'id','(')||aint(tree[0].first,'id','.')||aint(tree[0].first.first,'string','ADSAFE')||aint(tree[0].first.second,'string','lib')||tree[0].second.length!==2||tree[0].second[0].id!=='(string)'||aint(tree[0].second[1],'id','function'))){stop('adsafe_lib');}
if(tree.disrupt){warn('weird_program',prev_token);}}}
indent=null;advance('(end)');itself.property=property;}catch(e){if(e){JSLINT.errors.push({reason:e.message,line:e.line||next_token.line,character:e.character||next_token.from},null);}}
return JSLINT.errors.length===0;};itself.data=function(){var data={functions:[]},function_data,globals,i,j,kind,name,the_function,undef=[],unused=[];if(itself.errors.length){data.errors=itself.errors;}
...
return data;};itself.error_report=function(data){var evidence,i,output=[],snippets,warning;if(data.errors){if(data.json){output.push('<cite>JSON: bad.</cite><br>');}
...
if(data.unused||data['undefined']){output.push('<dl>');if(data['undefined']){output.push('<dt>undefined</dt><dd>');snippets=[];for(i=0;i<data['undefined'].length;i+=1){snippets[i]='<code>'+data['undefined'][i].name+'</code>&nbsp;<address>'+
data['undefined'][i]['function']+' '+
String(data['undefined'][i].line)+'</address>';}
output.push(snippets.join(', '));output.push('</dd>');}
if(data.unused){output.push('<dt>unused</dt><dd>');snippets=[];for(i=0;i<data.unused.length;i+=1){snippets[i]='<code>'+data.unused[i].name+'</code>&nbsp;<address>'+
data.unused[i]['function']+' '+
String(data.unused[i].line)+'</address>';}
output.push(snippets.join(', '));output.push('</dd>');}
output.push('</dl>');}
return output.join('');};itself.report=function(data){var dl,i,j,names,output=[],the_function;function detail(h,value){var comma_needed,singularity;if(Array.isArray(value)){output.push("<dt>"+h+"</dt><dd>");value.sort().forEach(function(item){if(item!==singularity){singularity=item;output.push((comma_needed?', ':'')+singularity);comma_needed=true;}});output.push("</dd>");}else if(value){output.push("<dt>"+h+"</dt><dd>",value,"</dd>");}}
output.push('<dl>');if(data.urls){detail('url',data.urls);dl=true;}
if(data.globals){detail('global',data.globals);dl=true;}else if(xmode==='style'){output.push("<dt>CSS.</dt>");}else if(data.json){if(!data.errors){output.push("<dt>JSON: good.</dt>");}}else{output.push("<dt><i>No new global variables introduced.</i></dt>");}
...
the_function.name.entityify()+'('+names.join(', ')+')');detail('undefined',the_function['undefined']);detail('unused',the_function.unused);detail('closure',the_function.closure);detail('variable',the_function['var']);detail('exception',the_function.exception);detail('outer',the_function.outer);detail('global',the_function.global);detail('label',the_function.label);output.push('</dl>');}}
return output.join('');};itself.properties_report=function(property){if(!property){return'';}
var i,key,keys=Object.keys(property).sort(),length,mem='    ',name,not_first=false,output=['/*properties'];for(i=0;i<keys.length;i+=1){key=keys[i];if(property[key]>0){if(not_first){mem+=', ';}
name=ix.test(key)?key:'\''+key.replace(nx,sanitize)+'\'';length+=name.length+2;if(mem.length+name.length>80){output.push(mem);mem='    ';}
mem+=name;not_first=true;}}
output.push(mem,'*/\n');return output.join('\n');};itself.jslint=itself;itself.edition='2012-11-17';return itself;}());