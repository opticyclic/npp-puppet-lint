...
return that;},true);infix('[',170,function(left,that){no_space_only(prev_token,token);no_space();step_in();edge();var e=expression(0),s;if(e.arity==='string'){if(option.safe&&(banned[e.value]||e.value.charAt(0)==='_'||e.value.slice(-1)==='_')){warn('adsafe_subscript_a',e);}else if(!option.evil&&(e.value==='eval'||e.value==='execScript')){warn('evil',e);}
tally_property(e.value);if(!option.sub&&ix.test(e.value)){s=syntax[e.value];if(!s||!s.reserved){warn('subscript',e);}}}else if(e.arity!=='number'&&option.safe){if(!((e.arity==='prefix'&&adsafe_prefix[e.id]===true)||(e.arity==='infix'&&adsafe_infix[e.id]===true))){warn('adsafe_subscript_a',e);}}
...
prefix('{',function(){var get,i,j,name,p,set,seen={};this.arity='prefix';this.first=[];step_in();while(next_token.id!=='}'){indent.wrap=false;edge();if(next_token.value==='get'&&peek().id!==':'){if(!option.es5){warn('get_set');}
...
output.push('</div>');}}
return output.join('');};itself.jslint=itself;itself.edition='2011-04-28';return itself;}());