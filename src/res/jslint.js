...
function nextLine(){var at;if(line>=lines.length){return false;}
character=1;s=lines[line];line+=1;at=s.search(/ \t/);if(at>=0){warningAt(bundle.mixed,line,at+1);}
s=s.replace(/\t/g,tab);at=s.search(cx);if(at>=0){warningAt(bundle.unsafe,line,at);}
if(option.maxlen&&option.maxlen<s.length){warningAt(bundle.too_long,line,s.length);}
return true;}
function it(type,value,quote){var i,t;if(type==='(color)'||type==='(range)'){t={type:type};}else if(type==='(punctuator)'||(type==='(identifier)'&&is_own(syntax,value))){t=syntax[value]||syntax['(error)'];}else{t=syntax[type];}
t=Object.create(t);if(type==='(string)'||type==='(range)'){if(jx.test(value)){warningAt(bundle.url,line,from);}}
if(type==='(identifier)'){t.identifier=true;if(value==='__iterator__'||value==='__proto__'){errorAt(bundle.reserved_a,line,from,value);}else if(option.nomen&&(value.charAt(0)==='_'||value.charAt(value.length-1)==='_')){warningAt(bundle.dangling_a,line,from,value);}}
...
line=0;nextLine();from=1;},range:function(begin,end){var c,value='';from=character;if(s.charAt(0)!==begin){errorAt(bundle.expected_a_b,line,character,begin,s.charAt(0));}
for(;;){s=s.slice(1);character+=1;c=s.charAt(0);switch(c){case'':errorAt(bundle.missing_a,line,character,c);break;case end:s=s.slice(1);character+=1;return it('(range)',value);case xquote:case'\\':warningAt(bundle.unexpected_a,line,character,c);break;}
...
function string(x){var c,j,r='';if(jsonmode&&x!=='"'){warningAt(bundle.expected_a,line,character,'"');}
if(xquote===x||(xmode==='scriptstring'&&!xquote)){return it('(punctuator)',x);}
function esc(n){var i=parseInt(s.substr(j+1,n),16);j+=n;if(i>=32&&i<=126&&i!==34&&i!==92&&i!==39){warningAt(bundle.unexpected_a,line,character,'\\');}
character+=n;c=String.fromCharCode(i);}
j=0;for(;;){while(j>=s.length){j=0;if(xmode!=='html'||!nextLine()){errorAt(bundle.unclosed,line,from);}}
c=s.charAt(j);if(c===x){character+=1;s=s.substr(j+1);return it('(string)',r,x);}
if(c<' '){if(c==='\n'||c==='\r'){break;}
warningAt(bundle.control_a,line,character+j,s.slice(0,j));}else if(c===xquote){warningAt(bundle.bad_html,line,character+j);}else if(c==='<'){if(option.safe&&xmode==='html'){warningAt(bundle.adsafe_a,line,character+j,c);}else if(s.charAt(j+1)==='/'&&(xmode||option.safe)){warningAt(bundle.expected_a_b,line,character,'<\\/','</');}else if(s.charAt(j+1)==='!'&&(xmode||option.safe)){warningAt(bundle.unexpected_a,line,character,'<!');}}else if(c==='\\'){if(xmode==='html'){if(option.safe){warningAt(bundle.adsafe_a,line,character+j,c);}}else if(xmode==='styleproperty'){j+=1;character+=1;c=s.charAt(j);if(c!==x){warningAt(bundle.unexpected_a,line,character,'\\');}}else{j+=1;character+=1;c=s.charAt(j);switch(c){case xquote:warningAt(bundle.bad_html,line,character+j);break;case'\\':case'"':case'/':break;case'\'':if(jsonmode){warningAt(bundle.unexpected_a,line,character,'\\\'');}
break;case'b':c='\b';break;case'f':c='\f';break;case'n':c='\n';break;case'r':c='\r';break;case't':c='\t';break;case'u':esc(4);break;case'v':if(jsonmode){warningAt(bundle.unexpected_a,line,character,'\\v');}
c='\v';break;case'x':if(jsonmode){warningAt(bundle.unexpected_a,line,character,'\\x');}
esc(2);break;default:warningAt(bundle.unexpected_a,line,character,'\\');}}}
r+=c;character+=1;j+=1;}}
...
if(s){if(xmode==='html'){return it('(error)',s.charAt(0));}else{errorAt(bundle.unexpected_a,line,character,s.substr(0,1));}}}else{if(c.isAlpha()||c==='_'||c==='$'){return it('(identifier)',t);}
if(c.isDigit()){if(xmode!=='style'&&!isFinite(Number(t))){warningAt(bundle.bad_number,line,character,t);}
if(xmode!=='style'&&xmode!=='styleproperty'&&s.substr(0,1).isAlpha()){warningAt(bundle.expected_space_a_b,line,character,c,s.charAt(0));}
if(c==='0'){d=t.substr(1,1);if(d.isDigit()){if(token.id!=='.'&&xmode!=='styleproperty'){warningAt(bundle.unexpected_a,line,character,t);}}else if(jsonmode&&(d==='x'||d==='X')){warningAt(bundle.unexpected_a,line,character,'0x');}}
if(t.substr(t.length-1)==='.'){warningAt(bundle.trailing_decimal_a,line,character,t);}
return it('(number)',t);}
switch(t){case'"':case"'":return string(t);case'//':if(src||(xmode&&xmode!=='script')){warningAt(bundle.unexpected_comment,line,character);}else if(xmode==='script'&&/<\s*\//i.test(s)){warningAt(bundle.unexpected_a,line,character,'<\/');}else if((option.safe||xmode==='script')&&ax.test(s)){warningAt(bundle.dangerous_comment,line,character);}
collect_comment(s);s='';break;case'/*':if(src||(xmode&&xmode!=='script'&&xmode!=='style'&&xmode!=='styleproperty')){warningAt(bundle.unexpected_comment,line,character);}
if(option.safe&&ax.test(s)){warningAt(bundle.dangerous_comment,line,character);}
for(;;){i=s.search(lx);if(i>=0){break;}
collect_comment(s);if(!nextLine()){errorAt(bundle.unclosed_comment,line,character);}else{if(option.safe&&ax.test(s)){warningAt(bundle.dangerous_comment,line,character);}}}
character+=i+2;if(s.substr(i,1)==='/'){errorAt(bundle.nested_comment,line,character);}
collect_comment(s.substr(0,i));s=s.substr(i+2);break;case'/*members':case'/*member':case'/*jslint':case'/*global':case'*/':return{value:t,type:'special',line:line,thru:character,from:from};case'':break;case'/':if(token.id==='/='){errorAt(bundle.slash_equal,line,from);}
if(prereg){depth=0;captures=0;l=0;for(;;){b=true;c=s.charAt(l);l+=1;switch(c){case'':errorAt(bundle.unclosed_regexp,line,from);return;case'/':if(depth>0){warningAt(bundle.unescaped_a,line,from+l,'/');}
c=s.substr(0,l-1);q={g:true,i:true,m:true};while(q[s.charAt(l)]===true){q[s.charAt(l)]=false;l+=1;}
character+=l;s=s.substr(l);q=s.charAt(0);if(q==='/'||q==='*'){errorAt(bundle.confusing_regexp,line,from);}
return it('(regexp)',c);case'\\':c=s.charAt(l);if(c<' '){warningAt(bundle.control_a,line,from+l,String(c));}else if(c==='<'){warningAt(bundle.unexpected_a,line,from+l,'\\');}
l+=1;break;case'(':depth+=1;b=false;if(s.charAt(l)==='?'){l+=1;switch(s.charAt(l)){case':':case'=':case'!':l+=1;break;default:warningAt(bundle.expected_a_b,line,from+l,':',s.charAt(l));}}else{captures+=1;}
break;case'|':b=false;break;case')':if(depth===0){warningAt(bundle.unescaped_a,line,from+l,')');}else{depth-=1;}
break;case' ':q=1;while(s.charAt(l)===' '){l+=1;q+=1;}
if(q>1){warningAt(bundle.use_braces,line,from+l,q);}
break;case'[':c=s.charAt(l);if(c==='^'){l+=1;if(option.regexp){warningAt(bundle.insecure_a,line,from+l,c);}else if(s.charAt(l)===']'){errorAt(bundle.unescaped_a,line,from+l,'^');}}
q=false;if(c===']'){warningAt(bundle.empty_class,line,from+l-1);q=true;}
klass:do{c=s.charAt(l);l+=1;switch(c){case'[':case'^':warningAt(bundle.unescaped_a,line,from+l,c);q=true;break;case'-':if(q){q=false;}else{warningAt(bundle.unescaped_a,line,from+l,'-');q=true;}
break;case']':if(!q){warningAt(bundle.unescaped_a,line,from+l-1,'-');}
break klass;case'\\':c=s.charAt(l);if(c<' '){warningAt(bundle.control_a,line,from+l,String(c));}else if(c==='<'){warningAt(bundle.unexpected_a,line,from+l,'\\');}
l+=1;q=true;break;case'/':warningAt(bundle.unescaped_a,line,from+l-1,'/');q=true;break;case'<':if(xmode==='script'){c=s.charAt(l);if(c==='!'||c==='/'){warningAt(bundle.html_confusion_a,line,from+l,c);}}
q=true;break;default:q=true;}}while(c);break;case'.':if(option.regexp){warningAt(bundle.insecure_a,line,from+l,c);}
break;case']':case'?':case'{':case'}':case'+':case'*':warningAt(bundle.unescaped_a,line,from+l,c);break;case'<':if(xmode==='script'){c=s.charAt(l);if(c==='!'||c==='/'){warningAt(bundle.html_confusion_a,line,from+l,c);}}
break;}
if(b){switch(s.charAt(l)){case'?':case'+':case'*':l+=1;if(s.charAt(l)==='?'){l+=1;}
break;case'{':l+=1;c=s.charAt(l);if(c<'0'||c>'9'){warningAt(bundle.expected_number_a,line,from+l,c);}
...
l+=1;high=+c+(high*10);}}}
if(s.charAt(l)!=='}'){warningAt(bundle.expected_a_b,line,from+l,'}',c);}else{l+=1;}
if(s.charAt(l)==='?'){l+=1;}
if(low>high){warningAt(bundle.not_greater,line,from+l,low,high);}
break;}}}
...
i=s.indexOf('<!');if(i>=0){errorAt(bundle.nested_comment,line,character+i);}
if(!nextLine()){errorAt(bundle.unclosed_comment,l,c);}}
l=s.indexOf('<!');if(l>=0&&l<i){errorAt(bundle.nested_comment,line,character+l);}
character+=i;if(s.charAt(i+2)!=='>'){errorAt(bundle.expected_a,line,character,'-->');}
...
if(t.length!==4&&t.length!==7){warningAt(bundle.bad_color_a,line,from+l,t);}
return it('(color)',t);}
...
if(!((c>='0'&&c<='9')||(c>='a'&&c<='z')||c==='#')){errorAt(bundle.bad_entity,line,from+l,character);}}
break;}
return it('(punctuator)',t);}}}}};}());function addlabel(t,type){if(option.safe&&funct['(global)']&&typeof predefined[t]!=='boolean'){warning(bundle.adsafe_a,token,t);}else if(t==='hasOwnProperty'){warning(bundle.bad_name_a,token,t);}
...
function doOption(){var b,obj,filter,o=nexttoken.value,t,v;switch(o){case'*/':error(bundle.unbegun_comment);break;case'/*members':case'/*member':o='/*members';if(!membersOnly){membersOnly={};}
obj=membersOnly;break;case'/*jslint':if(option.safe){warning(bundle.adsafe);}
obj=option;filter=boolOptions;break;case'/*global':if(option.safe){warning(bundle.adsafe);}
obj=predefined;break;default:error("What?");}
t=lex.token();loop:for(;;){for(;;){if(t.type==='special'&&t.value==='*/'){break loop;}
if(t.id!==','){break;}
t=lex.token();}
if(t.type!=='(string)'&&t.type!=='(identifier)'&&o!=='/*members'){error(bundle.unexpected_a,t);}
v=lex.token();if(v.id===':'){v=lex.token();if(obj===membersOnly){error(bundle.expected_a_b,t,'*/',':');}
if(t.value==='indent'&&o==='/*jslint'){b=+v.value;if(typeof b!=='number'||!isFinite(b)||b<0||Math.floor(b)!==b){error(bundle.expected_small_a,v,v.value);}
obj.white=true;obj.indent=b;}else if(t.value==='maxerr'&&o==='/*jslint'){b=+v.value;if(typeof b!=='number'||!isFinite(b)||b<=0||Math.floor(b)!==b){error(bundle.expected_small_a,v,v.value);}
obj.maxerr=b;}else if(t.value==='maxlen'&&o==='/*jslint'){b=+v.value;if(typeof b!=='number'||!isFinite(b)||b<0||Math.floor(b)!==b){error(bundle.expected_small_a,v,v.value);}
obj.maxlen=b;}else if(v.value==='true'){obj[t.value]=true;}else if(v.value==='false'){obj[t.value]=false;}else{error(bundle.unexpected_a,v);}
t=lex.token();}else{if(o==='/*jslint'){error(bundle.missing_option,t);}
obj[t.value]=false;t=v;}}
if(filter){assume();}}
function discard(){if(token.comments){if(nexttoken.comments){nexttoken.comments=nexttoken.comments.concat(token.comments);}else{nexttoken.comments=token.comments;}
token.comments=null;}
...
j+=1;}
return t;}
function expected_at(at){if(nexttoken.from!==at){warning(bundle.expected_a_at_b_c,nexttoken,nexttoken.value,at,nexttoken.from);}}
...
if(id&&nexttoken.id!==id){if(t){warning(bundle.expected_a_b_from_c_d,nexttoken,id,t.id,t.line,nexttoken.value);}else if(nexttoken.type!=='(identifier)'||nexttoken.value!==id){warning(bundle.expected_a_b,nexttoken,id,nexttoken.value);}}
...
function step_in(mode){var open,was;if(option.white&&option.indent){if(typeof mode==='number'){indent={at:mode,open:true,was:was};}else if(!indent){indent={at:1,mode:'statement',open:true};}else{was=indent;open=mode==='var'||(nexttoken.line!==token.line&&mode!=='statement');indent={at:(open||mode==='control'?was.at+option.indent:was.at)+
...
if(initial===true&&token.fud){left=token.fud();}else{if(token.nud){left=token.nud();}else{if(nexttoken.type==='(number)'&&token.id==='.'){warning(bundle.leading_decimal_a,token,nexttoken.value);advance();return token;}else{error(bundle.expected_identifier_a,token,token.id);}}
...
function expected_relation(node){if(node.assign){warning(bundle.conditional_assignment,node);}
return node;}
...
l=l.first;}while(l);}
if(left){if(left.id==='.'||left.id==='['){if(!left.first||left.first.value==='arguments'){warning(bundle.bad_assignment,that);}
that.second=expression(19);return that;}else if(left.identifier&&!left.reserved){if(funct[left.value]==='exception'){warning(bundle.assign_exception,left);}
that.second=expression(19);return that;}
if(left===syntax['function']){warning(bundle.identifier_function,token);}}
...
function optionalidentifier(){if(nexttoken.identifier){advance();if(option.safe&&banned[token.value]){warning(bundle.adsafe_a,token);}else if(token.reserved&&!option.es5){warning(bundle.expected_identifier_a_reserved,token);}
return token.value;}}
...
if(t.identifier&&!t.reserved&&peek().id===':'){edge('label');advance();advance(':');scope=Object.create(s);addlabel(t.value,'label');if(labelled[nexttoken.id]!==true){warning(bundle.label_a_b,nexttoken,t.value,nexttoken.value);}
if(jx.test(t.value+':')){warning(bundle.url,t);}
nexttoken.label=t;t=nexttoken;}
if(!no_indent){edge();}
step_in('statement');r=expression(0,true);if(r.arity==='statement'){if(r.id!=='switch'&&(!r.block||r.id==='do')){semicolon();}else{spaces();}}else{if(r.id==='('&&r.first.id==='new'){warning(bundle.bad_new);}else if(!r.assign&&r.id!=='delete'&&r.id!=='++'&&r.id!=='--'&&r.id!=='('){warning(bundle.assignment_function_expression,token);}
...
return a;}
function countMember(m){if(membersOnly&&typeof membersOnly[m]!=='boolean'){warning(bundle.unexpected_member_a,token,m);}
...
return this;});prefix('typeof');prefix('new',function(){one_space();var c=expression(160),i;if(c.id!=='function'){if(c.identifier){switch(c.value){case'Object':warning(bundle.use_object,token);break;case'Array':if(nexttoken.id!=='('){warning(bundle.use_array,token);}else{advance('(');if(nexttoken.id===')'){warning(bundle.use_array,token);}
advance(')');}
this.first=c;return this;case'Number':case'String':case'Boolean':case'Math':case'JSON':warning(bundle.not_a_constructor,c);break;case'Function':if(!option.evil){warning(bundle.function_eval);}
...
if(!option.evil){if(left.value==='eval'||left.value==='Function'||left.value==='execScript'){warning(bundle.evil,left);}else if(p[0]&&p[0].id==='(string)'&&(left.value==='setTimeout'||left.value==='setInterval')){warning(bundle.implied_evil,left);}}
...
return that;},true);infix('[',170,function(left,that){no_space_only(prevtoken,token);no_space();step_in();edge();var e=expression(0),s;if(e.type==='(string)'){if(option.safe&&banned[e.value]===true){warning(bundle.adsafe_a,e);}else if(!option.evil&&(e.value==='eval'||e.value==='execScript')){warning(bundle.evil,e);}else if(option.safe&&(e.value.charAt(0)==='_'||e.value.charAt(0)==='-')){warning(bundle.adsafe_subscript_a,e);}
countMember(e.value);if(!option.sub&&ix.test(e.value)){s=syntax[e.value];if(!s||!s.reserved){warning(bundle.subscript,e);}}}else if(e.type!=='(number)'||e.value<0){if(option.safe){warning(bundle.adsafe_subscript_a,e);}}
...
step_out(']',this);discard();return this;},170);function property_name(){var id=optionalidentifier(true);if(!id){if(nexttoken.id==='(string)'){id=nexttoken.value;if(option.safe){if(banned[id]){warning(bundle.adsafe_a);}else if(id.charAt(0)==='_'||id.charAt(id.length-1)==='_'){warning(bundle.dangling_a);}}
...
for(;;){edge();i=identifier();p.push(token);addlabel(i,'parameter');if(nexttoken.id===','){comma();}else{no_space();step_out(')',t);discard();return p;}}}
...
func.name=name||'';func.first=funct['(params)']=functionparams();func.block=block(false);scope=s;funct=funct['(context)'];return func;}
...
doFunction(get,'');if(funct['(loopage)']){warning(bundle.function_loop,t);}
...
doFunction(set,'');p=set.first;if(!p||p.length!==1||p[0]!=='value'){warning(bundle.parameter_set_a,t,i);}
...
addlabel(id,'unused');if(nexttoken.id==='='){assign=nexttoken;assign.first=name;spaces();advance('=');spaces();if(nexttoken.id==='undefined'){warning(bundle.unnecessary_initialize,token,id);}
...
doFunction(this,i,true);if(nexttoken.id==='('&&nexttoken.line===token.line){error(bundle.function_statement);}
this.arity='statement';return this;});prefix('function',function(){one_space();var i=optionalidentifier();if(i){no_space_only();}
doFunction(this,i);if(funct['(loopage)']){warning(bundle.function_loop);}
this.arity='function';return this;});stmt('if',function(){var t=nexttoken;one_space();advance('(');step_in('control');discard();no_space();edge();this.arity='statement';this.first=expected_relation(expression(0));no_space();step_out(')',t);discard();one_space();this.block=block(true);if(nexttoken.id==='else'){one_space();advance('else');discard();one_space();this['else']=nexttoken.id==='if'||nexttoken.id==='switch'?statement(true):block(true);if(this['else'].disrupt&&this.block.disrupt){this.disrupt=true;}}
return this;});stmt('try',function(){var b,e,s,t;if(option.adsafe){warning(bundle.adsafe_a,this);}
one_space();this.arity='statement';this.block=block(false);if(nexttoken.id==='catch'){one_space();advance('catch');one_space();t=nexttoken;advance('(');step_in('control');discard();no_space();edge();s=scope;scope=Object.create(s);e=nexttoken.value;this.first=e;if(nexttoken.type!=='(identifier)'){warning(bundle.expected_identifier_a,nexttoken);}else{addlabel(e,'exception');}
...
return this;});stmt('while',function(){one_space();var t=nexttoken;funct['(breakage)']+=1;funct['(loopage)']+=1;advance('(');step_in('control');discard();no_space();edge();this.arity='statement';this.first=expected_relation(expression(0));no_space();step_out(')',t);discard();one_space();this.block=block(true);if(this.block.disrupt){warning(bundle.strange_loop,prevtoken);}
funct['(breakage)']-=1;funct['(loopage)']-=1;return this;});reserve('with');stmt('switch',function(){var b=true,s,t=nexttoken;funct['(breakage)']+=1;one_space();advance('(');discard();no_space();step_in();this.arity='statement';this.first=expected_relation(expression(0));if(this.first.id==='NaN'){warning(bundle.unexpected_a,this.first);}
no_space();step_out(')',t);discard();one_space();advance('{');step_in();this.second=[];while(nexttoken.id==='case'){t=nexttoken;t.first=[];spaces();edge('case');advance('case');for(;;){one_space();s=expression(0);t.first.push(s);if(s.id==='NaN'){warning(bundle.unexpected_a,s);}
...
advance();i=nexttoken;advance('in');i.first=v;i.second=expression(20);step_out(')',t);discard();this.first=i;s=block(true);if(!f&&(s.length>1||typeof s[0]!=='object'||s[0].value!=='if'||s[0].first.id==='true')){warning(bundle.for_if,this);}}else{if(nexttoken.id!==';'){edge();this.first=[];for(;;){this.first.push(expression(0,'for'));if(nexttoken.id!==','){break;}
comma();}}
semicolon();if(nexttoken.id!==';'){edge();this.second=expected_relation(expression(0));}
...
this.block=s;funct['(breakage)']-=1;funct['(loopage)']-=1;return this;});disruptstmt('break',function(){var v=nexttoken.value;this.arity='statement';if(funct['(breakage)']===0){warning(bundle.unexpected_a,this);}
...
return this;});disruptstmt('continue',function(){var v=nexttoken.value;this.arity='statement';if(funct['(breakage)']===0){warning(bundle.unexpected_a,this);}
if(nexttoken.identifier&&token.line===nexttoken.line){one_space_only();if(funct[v]!=='label'){warning(bundle.not_a_label,nexttoken);}else if(scope[v]!==funct){warning(bundle.not_a_scope,nexttoken);}
this.first=nexttoken;advance();}
return this;});disruptstmt('return',function(){this.arity='statement';if(nexttoken.id!==';'&&nexttoken.line===token.line){one_space_only();if(nexttoken.id==='/'||nexttoken.id==='(regexp)'){warning(bundle.wrap_regexp);}
this.first=expression(20);}
return this;});disruptstmt('throw',function(){this.arity='statement';one_space_only();this.first=expression(20);return this;});reserve('void');reserve('class');reserve('const');reserve('enum');reserve('export');reserve('extends');reserve('import');reserve('super');reserve('let');reserve('yield');reserve('implements');reserve('interface');reserve('package');reserve('private');reserve('protected');reserve('public');reserve('static');function jsonValue(){function jsonObject(){var o={},t=nexttoken;advance('{');if(nexttoken.id!=='}'){while(nexttoken.id!=='(end)'){while(nexttoken.id===','){warning(bundle.unexpected_a,nexttoken);comma();}
if(nexttoken.id!=='(string)'){warning(bundle.expected_string_a);}
...
advance('}',t);}
function jsonArray(){var t=nexttoken;advance('[');if(nexttoken.id!==']'){while(nexttoken.id!=='(end)'){while(nexttoken.id===','){warning(bundle.unexpected_a,nexttoken);comma();}
jsonValue();if(nexttoken.id!==','){break;}
comma();if(nexttoken.id===']'){warning(bundle.unexpected_a,token);break;}}}
advance(']',t);}
switch(nexttoken.id){case'{':jsonObject();break;case'[':jsonArray();break;case'true':case'false':case'null':case'(number)':case'(string)':advance();break;case'-':advance('-');no_space_only();advance('(number)');break;default:error(bundle.unexpected_a);}}
...
function cssColor(){var i,number,t,value;if(nexttoken.identifier){value=nexttoken.value;if(value==='rgb'||value==='rgba'){advance();t=nexttoken;advance('(');for(i=0;i<3;i+=1){if(i){comma();}
number=nexttoken.value;if(nexttoken.type!=='(number)'||number<0){warning(bundle.expected_positive_a,nexttoken);advance();}else{advance();if(nexttoken.id==='%'){advance('%');if(number>100){warning(bundle.expected_percent_a,token,number);}}else{if(number>255){warning(bundle.expected_small_a,token,number);}}}}
if(value==='rgba'){comma();number=+nexttoken.value;if(nexttoken.type!=='(number)'||number<0||number>1){warning(bundle.expected_fraction_a,nexttoken);}
advance();if(nexttoken.id==='%'){warning(bundle.unexpected_a);advance('%');}}
advance(')',t);return true;}else if(cssColorData[nexttoken.value]===true){advance();return true;}}else if(nexttoken.type==='(color)'){advance();return true;}
return false;}
function cssLength(){if(nexttoken.id==='-'){advance('-');no_space_only();}
if(nexttoken.type==='(number)'){advance();if(nexttoken.type!=='(string)'&&cssLengthData[nexttoken.value]===true){no_space_only();advance();}else if(+token.value!==0){warning(bundle.expected_linear_a);}
return true;}
return false;}
...
function cssAttr(){if(nexttoken.identifier&&nexttoken.value==='attr'){advance();advance('(');if(!nexttoken.identifier){warning(bundle.expected_name_a);}
advance();advance(')');return true;}
return false;}
function cssCommaList(){while(nexttoken.id!==';'){if(!cssName()&&!cssString()){warning(bundle.expected_name_a);}
if(nexttoken.id!==','){return true;}
comma();}}
function cssCounter(){if(nexttoken.identifier&&nexttoken.value==='counter'){advance();advance('(');advance();if(nexttoken.id===','){comma();if(nexttoken.type!=='(string)'){warning(bundle.expected_string_a);}
advance();}
advance(')');return true;}
if(nexttoken.identifier&&nexttoken.value==='counters'){advance();advance('(');if(!nexttoken.identifier){warning(bundle.expected_name_a);}
advance();if(nexttoken.id===','){comma();if(nexttoken.type!=='(string)'){warning(bundle.expected_string_a);}
advance();}
if(nexttoken.id===','){comma();if(nexttoken.type!=='(string)'){warning(bundle.expected_string_a);}
advance();}
advance(')');return true;}
return false;}
function cssShape(){var i;if(nexttoken.identifier&&nexttoken.value==='rect'){advance();advance('(');for(i=0;i<4;i+=1){if(!cssLength()){warning(bundle.expected_number_a);break;}}
advance(')');return true;}
return false;}
function cssUrl(){var c,url;if(nexttoken.identifier&&nexttoken.value==='url'){nexttoken=lex.range('(',')');url=nexttoken.value;c=url.charAt(0);if(c==='"'||c==='\''){if(url.slice(-1)!==c){warning(bundle.bad_url);}else{url=url.slice(1,-1);if(url.indexOf(c)>=0){warning(bundle.bad_url);}}}
...
return false;}
cssAny=[cssUrl,function(){for(;;){if(nexttoken.identifier){switch(nexttoken.value.toLowerCase()){case'url':cssUrl();break;case'expression':warning(bundle.unexpected_a);advance();break;default:advance();}}else{if(nexttoken.id===';'||nexttoken.id==='!'||nexttoken.id==='(end)'||nexttoken.id==='}'){return true;}
...
advance();return cssAny;}else{if(!nexttoken.identifier){warning(bundle.expected_style_attribute);}else{if(is_own(cssAttributeData,nexttoken.value)){v=cssAttributeData[nexttoken.value];}else{v=cssAny;if(!option.css){warning(bundle.unrecognized_style_attribute_a);}}}
advance();return v;}}
...
v=styleAttribute();advance(':');if(nexttoken.identifier&&nexttoken.value==='inherit'){advance();}else{if(!styleValue(v)){warning(bundle.unexpected_a);advance();}}
...
function styleSelector(){if(nexttoken.identifier){if(!is_own(htmltag,option.cap?nexttoken.value.toLowerCase():nexttoken.value)){warning(bundle.expected_tagname_a);}
...
advance(')');break;case'nth-child':case'nth-last-child':case'nth-last-of-type':case'nth-of-type':advance();advance('(');styleChild();advance(')');break;case'not':advance();advance('(');if(nexttoken.id===':'&&peek(0).value==='not'){warning(bundle.not);}
styleSelector();advance(')');break;default:warning(bundle.expected_pseudo_a);}
...
advance();if(nexttoken.id==='='||nexttoken.value==='~='||nexttoken.value==='$='||nexttoken.value==='|='||nexttoken.id==='*='||nexttoken.id==='^='){advance();if(nexttoken.type!=='(string)'){warning(bundle.expected_string_a);}
advance();}
advance(']');break;default:error(bundle.expected_selector_a);}}}
function stylePattern(){if(nexttoken.id==='{'){warning(bundle.expected_style_pattern);}
...
function styles(){var i;while(nexttoken.id==='@'){i=peek();advance('@');if(nexttoken.identifier){switch(nexttoken.value){case'import':advance();if(!cssUrl()){warning(bundle.expected_a_b,nexttoken,'url',nexttoken.value);advance();}
semicolon();break;case'media':advance();for(;;){if(!nexttoken.identifier||cssMedia[nexttoken.value]===true){error(bundle.expected_media_a);}
advance();if(nexttoken.id!==','){break;}
comma();}
advance('{');stylelist();advance('}');break;default:warning(bundle.expected_at_a);}}else{warning(bundle.expected_at_a);}}
stylelist();}
function doBegin(n){if(n!=='html'&&!option.fragment){if(n==='div'&&option.adsafe){error(bundle.adsafe_fragment);}else{error(bundle.expected_a_b,token,'html',n);}}
...
function doAttribute(n,a,v){var u,x;if(a==='id'){u=typeof v==='string'?v.toUpperCase():'';if(ids[u]===true){warning(bundle.duplicate_a,nexttoken,v);}
...
function doTag(n,a){var i,t=htmltag[n],x;src=false;if(!t){error(bundle.unrecognized_tag_a,nexttoken,n===n.toLowerCase()?n:n+' (capitalization error)');}
...
step_in(nexttoken.from);use_strict();statements('script');indent=null;}
...
v=htmltag[n];if(typeof v!=='object'){error(bundle.unrecognized_tag_a,t,n);}
...
xmode='styleproperty';xquote=q;advance(q);substyle();xmode='html';xquote='';advance(q);v=false;}else{if(nexttoken.id==='='){advance('=');v=nexttoken.value;if(!nexttoken.identifier&&nexttoken.id!=='"'&&nexttoken.id!=='\''&&nexttoken.type!=='(string)'&&nexttoken.type!=='(number)'&&nexttoken.type!=='(color)'){warning(bundle.expected_attribute_value_a,token,a);}
advance();}else{v=true;}}
...
global=Object.create(predefined);scope=global;funct={'(global)':true,'(name)':'(global)','(scope)':scope,'(breakage)':0,'(loopage)':0};functions=[funct];ids={};urls=[];src=false;xmode=false;stack=null;member={};membersOnly=null;implied={};inblock=false;indent=false;lookahead=[];jsonmode=false;warnings=0;lex.init(s);prereg=true;strict_mode=false;var_mode=false;prevtoken=token=nexttoken=syntax['(begin)'];assume();try{advance();if(nexttoken.value.charAt(0)==='<'){html();if(option.adsafe&&!adsafe_went){warning(bundle.adsafe_go,this);}}else{switch(nexttoken.id){case'{':case'[':jsonmode=true;jsonValue();break;case'@':case'*':case'#':case'.':case':':xmode='style';advance();if(token.id!=='@'||!nexttoken.identifier||nexttoken.value!=='charset'||token.line!==1||token.from!==1){error(bundle.css);}
advance();if(nexttoken.type!=='(string)'&&nexttoken.value!=='UTF-8'){error(bundle.css);}
...
o.push(m+'<br>*/</pre>');}
o.push('</div>');}}
return o.join('');};itself.jslint=itself;itself.edition='2011-01-28';return itself;}());